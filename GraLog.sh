#Ejemplo: GraLog.sh MoverA.sh 'Moviendo archivo de origen a destino' 'INFO'
...
#Si GraLog.sh es llamado sin la cantidad de comandos correctos mostrar mensaje
if [ $# -ne 3 -a $# -ne 2 ]
then
	echo "Gralog.sh no ha sido llamado correctamente. El formato de llamada es:"
	echo "GraLog.sh ComandoQueLlamo 'Mensaje' TipoDeMensaje(opcional)"
	echo `date +%F`"|"`date +%T`" $user $nombreScript ERR Comando Gralog.sh mal utilizado" >> "$loggerPathDefault"
